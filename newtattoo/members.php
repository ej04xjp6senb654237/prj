                       ...
                       <?php
							...
							if (mysql_num_rows($rs)>0) { 
								...
								for ($i=0; $i<$total; $i++)      {
									...
									echo  ' <span class="icon"></span></a> 會員名稱:'.$row["姓名"].'<br>'.' 會員地址:'.$row["地址"].'<br>'.'會員手機號碼:'.$row["手機號碼"].'</a>';
									echo "</li>";
								}
							}
							$SQLStr = "SELECT * FROM 訂單 where 帳號='".$_SESSION['ADMINUSER']."'";//." and 會員.密碼='".$_SESSION['ADMINPWD']."'";
							$rs = db_query( $SQLStr  );   
							//echo("共有".mysql_num_rows($rs)."個商品*****");
							if (mysql_num_rows($rs)>0) { 
								$total = mysql_num_rows($rs); 
								for ($i=0; $i<$total; $i++)      {
									...
									//****提示**** 以下『一條』指令要改	
									//****提示**** 商品照片的檔名在 $row["圖檔名"] 裡、單價在 $row["價錢"] 裡、商品的名稱在 $row["商品名稱"] 裡
									echo  ' <span class="icon"></span></a> 訂單編號:'.$row["訂單編號"].'<br>'.' 訂購日期:'.$row["訂購日期"].'</a>';
									echo "</li>";
								}
							}
						?>                        
